...
/*
 * Recomendado:leer algunas transparencias donde con dibujos muy sencillos se muestran las operaciones
 * relacionadas con listas simplemente enlazadas:
...
typedef struct Automovil{
	char patente[7];		//3 letras 3 nros y un guion
	char vin[18];			//17 digitos y letras para nro.grabado en el motor
	char* marca;
	char* modelo;
	int potencia;			//en alguna unidad
	char* color;
}Auto;
...
//nodo de un lista simplemente enlazada
typedef struct Nodo{
	Auto *a;
	struct Nodo* next;//puntero al siguiente nodo que contiene un auto
}nodo;

//inserta un nodo en la cabeza de la lista (principio), el final se indica con NULL, recibe puntero al comienzo y al nuevo nodo
//para poder modificar el puntero mismo debo pasar su direccion,por eso paso **, recuerde que en C se pasan argumentos por valor
//aqui se va modificando la cabeza de la lista con cada nodo que agrego al principio
/*Algoritmo Insertar Nodo_{_	p = new Nodo
_	info(p) = Elemento de datos a insertar
	_	sig(p) = inicio
		_	inicio = p_}
*/
...
int E10_main()
{
	Auto a={"ABC123","1HGBH41JXMN109186","Fiat","Fiorino",44,"rojo alpine"};
	Auto b={"JHG776","VF33CN6AP1Y400321","Renault", "Clio",76,"azul crepusculo"};
	Auto c={"AHU234","8AD3CN6AP4G003230","Volkswagen","Fox Highline",110,"negro"};
